...
const Hero = () => {
  ...
  return (
    <div className="block relative lg:flex max-w-5xl mx-auto mt-2 md:mt-20 px-0 md:px-2">
      <div>
        <h1 className="font-bold text-3xl md:text-5xl leading-tight text-zinc-50 max-w-3xl">
          {hero.heading.split1} <span className="text-secondary-default">{hero.heading.split2}</span>
        </h1>
      ...
      <div className="mt-4 lg:mt-0 p-3 lg:pl-5 ">
        ...
        <div className={`${styles.flexCenter} space-x-2 mt-2`}>
          {socialMediaLinks.map(({ id, url, icon, name }) => (
            <Link href={url} key={id} className="text-zinc-500 text-sm relative hover:text-secondary-default">
